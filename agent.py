...
def build_turn_cap_response(results: list[dict[str, Any]]) -> dict[str, Any]:
    recs = retriever.recommendations_from_results(results, limit=10)
    if recs:
        return {
            "reply": (
...
def _call_llm_sync(prompt: str) -> str:
    ...
    model_ids = [GEMINI_MODEL_PRIMARY, *GEMINI_MODEL_FALLBACKS]
    ...
    for model_id in model_ids:
        try:
            ...
            if model_id != GEMINI_MODEL_PRIMARY:
                logger.info("LLM used fallback model: %s", model_id)
...
def validate_and_strip_hallucinations(parsed: dict[str, Any]) -> dict[str, Any]:
    ...
    for rec in recs:
        ...
        if catalog_item and url not in seen_urls:
    ...
    if removed:
        logger.warning(
            "Removed %d invalid or mismatched recommendation(s)", removed
        )
...
def ensure_recommendations_when_ready(
    ...
    if is_compare or (is_first_user_turn(messages) and query_is_vague(messages)):
        return parsed
    ...
    if should_recommend:
        parsed["recommendations"] = retriever.recommendations_from_results(results, limit=10)
...
def build_retrieval_fallback(
    ...
    if is_compare:
        ...
        for msg in reversed(messages):
            if msg.get("role") == "user":
                ...
                break
        found = retriever.find_names_in_text(last_user, max_names=4)
        if len(found) >= 2:
            names = " and ".join(item["name"] for item in found[:2])
            return {
                "reply": (
                    f"I found {names} in the catalog, but I cannot generate a detailed "
                    "comparison right now. Please try again shortly."
                ),
                "recommendations": [],
                "end_of_conversation": False,
            }
        return {
            "reply": (
                "I don't have that assessment in my catalog, or I could not generate "
                "a comparison right now. Please check the exact SHL product names."
            ),
            "recommendations": [],
            "end_of_conversation": False,
        }
    ...
    if user_turn >= 6 or (user_turn >= MAX_USER_TURNS - 1 and has_recommendation_context(messages)):
        recs = retriever.recommendations_from_results(results, limit=10)
        if recs:
            end = user_turn >= MAX_USER_TURNS
            return {
                "reply": (
                    "Based on our conversation, here are SHL assessments from the catalog "
                    "that best match your hiring needs."
                ),
                "recommendations": recs,
                "end_of_conversation": end,
            }

    if results and has_recommendation_context(messages):
        recs = retriever.recommendations_from_results(results, limit=10)
        return {
            "reply": (
                "Here are SHL assessments from our catalog that may match your needs. "
                "Tell me if you'd like to refine this list."
            ),
            "recommendations": recs,
            "end_of_conversation": False,
        }
...
def _fetch_results(messages: list[dict], is_compare: bool) -> list[dict[str, Any]]:
    if is_compare:
        return []
...
def run(messages: list[dict]) -> dict:
    ...
    if user_turn > MAX_USER_TURNS:
        results = _fetch_results(messages, False)
    ...
    results = _fetch_results(messages, is_compare)
    ...
    mode_hint = ""
    if detect_refine_intent(messages):
        mode_hint = (
            "\n\nIMPORTANT: User is refining a prior shortlist. Use REFINE mode. "
            "Return an updated 1-10 item shortlist from the CATALOG. Do not ask "
            "clarifying questions.\n"
        )
    elif user_turn >= 6 and has_recommendation_context(messages):
        mode_hint = (
            "\n\nIMPORTANT: User turn is "
            f"{user_turn}/{MAX_USER_TURNS}. You MUST RECOMMEND now (1-10 items) "
            "using the CATALOG.\n"
        )
    ...
    if _is_llm_fallback(parsed):
        ...
        if is_first_user_turn(messages) and query_is_vague(messages):
            parsed["recommendations"] = []
        elif user_turn >= MAX_USER_TURNS and parsed.get("recommendations"):
            parsed["end_of_conversation"] = True
        parsed = ensure_recommendations_when_ready(
            parsed, messages, results, user_turn, is_compare
        )
